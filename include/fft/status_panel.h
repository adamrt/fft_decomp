#ifndef FFT_STATUS_PANEL_H
#define FFT_STATUS_PANEL_H

#include "fft/menu_types.h"
#include "psx/gpu.h"
#include "psx/types.h"

/* Unit status and character-editor panel records. The EVENT overlays
 * (ATTACK, BUNIT, DEBUGCHR, EQUIP, REQUIRE) each link their own byte-for-byte
 * twin of the panel code, and the WORLD/BATTLE unit-editor panels reuse the
 * same packet layouts, so the types are named neutrally here and each
 * overlay's matching builders prove the offsets. */

/* libgpu DR_OFFSET packet (the DR_MODE layout) followed by the x/y pair that
 * SetDrawOffset() reads back into it. */
typedef struct status_panel_draw_offset {
    u32 tag;     /* 0x00 */
    u32 code[2]; /* 0x04 */
    s16 x;       /* 0x0c */
    s16 y;       /* 0x0e */
} status_panel_draw_offset_t;

typedef char status_panel_draw_offset_size_must_be_0x10[(sizeof(status_panel_draw_offset_t) == 0x10) ? 1 : -1];
...
typedef struct status_panel_offset_pair {
    u16 x; /* 0x00 */
    u16 y; /* 0x02 */
} status_panel_offset_pair_t;
...
typedef struct status_panel_menu_primitives {
    DR_MODE draw_modes[3]; /* 0x000 */
    u8 unknown_24[0x204 - 0x24];
    TILE tiles[2];    /* 0x204 */
    LINE_F2 lines[8]; /* 0x224 */
} status_panel_menu_primitives_t;

typedef char
    status_panel_menu_primitives_size_must_be_0x2a4[(sizeof(status_panel_menu_primitives_t) == 0x2a4) ? 1 : -1];
 ...
 * status_panel_buffer_t.sprites[n]. */
typedef struct status_panel_primitives {
    u8 header[0x10];
    SPRT sprites[25]; /* 0x010 */
} status_panel_primitives_t;

typedef char status_panel_primitives_size_must_be_0x204[(sizeof(status_panel_primitives_t) == 0x204) ? 1 : -1];
...
typedef struct status_panel_indicator_prims {
    DR_MODE mode;   /* 0x00 */
    TILE frame;     /* 0x0c */
    TILE top[9];    /* 0x1c */
    TILE bottom[9]; /* 0xac */
} status_panel_indicator_prims_t;

typedef char
    status_panel_indicator_prims_size_must_be_0x13c[(sizeof(status_panel_indicator_prims_t) == 0x13c) ? 1 : -1];
...
typedef struct status_panel_portrait_primitive_tail {
    DR_AREA areas[2]; /* 0x00 */
    RECT rects[2];    /* 0x18 */
} status_panel_portrait_primitive_tail_t;

typedef char status_panel_portrait_primitive_tail_size_must_be_0x28
    [(sizeof(status_panel_portrait_primitive_tail_t) == 0x28) ? 1 : -1];
 ...
 *    status_panel_menu_primitives_t describes;
 ...
 *    pair. */
typedef struct status_panel_buffer {
    DR_MODE draw_modes[3];                           /* 0x000 */
    ...
    status_panel_portrait_primitive_tail_t portrait; /* 0x3b0 */
} status_panel_buffer_t;

typedef char status_panel_buffer_size_must_be_0x3d8[(sizeof(status_panel_buffer_t) == 0x3d8) ? 1 : -1];
...
typedef struct status_panel_numeric_buffer {
    DR_MODE draw_modes[3];                           /* 0x000 */
    SPRT sprites[18];                                /* 0x024 */
    world_menu_palette_primitives_t numeric_frame;   /* 0x18c */
    status_panel_draw_offset_t draw_offsets[2];      /* 0x278 */
    status_panel_portrait_primitive_tail_t portrait; /* 0x298 */
} status_panel_numeric_buffer_t;

typedef char status_panel_numeric_buffer_size_must_be_0x2c0[(sizeof(status_panel_numeric_buffer_t) == 0x2c0) ? 1 : -1];
...
typedef struct status_panel_frame_config {
    u16 origin_x; /* 0x00 */
    u8 unknown_02[2];
    u16 origin_y; /* 0x04 */
    u8 unknown_06[6];
...
typedef char status_panel_frame_config_size_must_be_0x14[(sizeof(status_panel_frame_config_t) == 0x14) ? 1 : -1];

/* u16 view of the -1 stored in an empty status_panel_slot_state_t.ability_ids row. */
enum {
    STATUS_PANEL_LABEL_NONE = 0xffff,
};
...
typedef struct status_panel_slot_state {
    u8 unknown_00[0x0e];
...
} status_panel_slot_state_t;

typedef char status_panel_slot_state_size_must_be_0x38[(sizeof(status_panel_slot_state_t) == 0x38) ? 1 : -1];
...
typedef struct status_panel_gauges {
    s16 level;      /* 0x00 */
    s16 team_state; /* 0x02 */
    s16 _04;
    s16 _06;
    ...
    s16 _1a;
    s16 max_ct; /* 0x1c */
} status_panel_gauges_t;

typedef char status_panel_gauges_size_must_be_0x1e[(sizeof(status_panel_gauges_t) == 0x1e) ? 1 : -1];

/* One value bar: the Gouraud bar is drawn value/limit wide. */
typedef struct status_panel_bar {
    s16 value;
    s16 unknown_02;
    s16 limit; /* 0x04; a zero limit draws the fixed 4-pixel stub */
} status_panel_bar_t;

typedef char status_panel_bar_size_must_be_6[(sizeof(status_panel_bar_t) == 6) ? 1 : -1];
...
typedef struct status_panel_editor_state {
    ...
    u8 unknown_09;
    s16 unit;                   /* 0x0a */
    status_panel_bar_t bars[3]; /* 0x0c */
    s16 unknown_1E[3];
} status_panel_editor_state_t;

typedef char status_panel_editor_state_size_must_be_0x24[(sizeof(status_panel_editor_state_t) == 0x24) ? 1 : -1];
...
typedef struct status_panel_numeric_entry {
    u8 unknown_00[8];
    s16 format; /* 0x08: digits argument; low byte digit count, high byte sign/marker
                   flags (0xc00/0xe00 draw dashes) */
    u8 unknown_0A[2];
} status_panel_numeric_entry_t;

typedef char status_panel_numeric_entry_size_must_be_0xc[(sizeof(status_panel_numeric_entry_t) == 0xc) ? 1 : -1];
...
typedef struct status_panel_editor_packet {
...
} status_panel_editor_packet_t;

typedef char status_panel_editor_packet_size_must_be_0x30c[(sizeof(status_panel_editor_packet_t) == 0x30c) ? 1 : -1];
...
typedef struct status_panel_packet {
    u8 unknown_000[0xec];                     /* numeric display frame */
...
} status_panel_packet_t;

typedef char status_panel_packet_size_must_be_0x1d8[(sizeof(status_panel_packet_t) == 0x1d8) ? 1 : -1];
...
typedef struct status_panel_display_thread {
    u16 x; /* 0x00 */
    u16 unknown_02;
    union {
        s32 word; /* 0x04 */
...
} status_panel_display_thread_t;

/* Item graphic load descriptor filled by *_gfx_build_item_graphic_descriptor. */
typedef struct status_panel_graphic_descriptor {
    s16 x_load_location;
    s16 y_load_location;
    s16 width;
    s16 height;
    u16 clut;
    s16 tpage;
} status_panel_graphic_descriptor_t;

typedef char
    status_panel_graphic_descriptor_size_must_be_0xc[(sizeof(status_panel_graphic_descriptor_t) == 0xc) ? 1 : -1];

/* Ordering-tag/intensity/scale record read by the
 * *_gfx_build_scaled_sprite_primitive_chain builders. */
typedef struct status_panel_scaled_sprite {
    u32* ordering_tag;
    u8 intensity;
    u8 pad_05[3];
    s32 link_primitive;
    ...
    u8 pad_10[8];
    s16 offset[6];
} status_panel_scaled_sprite_t;

typedef char status_panel_scaled_sprite_size_must_be_0x24[(sizeof(status_panel_scaled_sprite_t) == 0x24) ? 1 : -1];

/* Glyph source rectangle used by the *_text_render_decimal_value renderers. */
typedef struct status_panel_glyph {
    s16 source_x;
    s16 source_y;
    s16 width;
    s16 height;
} status_panel_glyph_t;
...
typedef struct status_panel_gauge_entry {
    u16 x;
    u16 y;
    s16* value;
    s16 flags;
    u16 unknown_0a;
} status_panel_gauge_entry_t;

typedef char status_panel_gauge_entry_size_must_be_0xc[(sizeof(status_panel_gauge_entry_t) == 0xc) ? 1 : -1];
 ...
 * the sign glyphs. */
typedef struct status_panel_text_position {
    u16 x;
    u16 y;
    u8 unknown_04[4];
    u16 row_stride; /* 0x08 */
    u16 unknown_0a;
    u32 color; /* 0x0c */
} status_panel_text_position_t;

typedef char status_panel_text_position_size_must_be_0x10[(sizeof(status_panel_text_position_t) == 0x10) ? 1 : -1];
