...
void card_gfx_build_scaled_sprite_primitive_chain(battle_menu_status_panel_scaled_sprite_t* sprite, POLY_FT4* poly) {
    ...
    for (i = 0; i < g_card_gfx_scaled_sprite_count; i++) {
        SetPolyFT4(poly);
        ...
        SetSemiTrans(poly, 1);
        SetShadeTex(poly, 0);
        x = g_card_gfx_scaled_sprite_scale[i].x;
        x *= sprite->scale_x;
        y = g_card_gfx_scaled_sprite_scale[i].y;
        y *= sprite->scale_y;
        if (x % ONE >= 0x800) {
            x += 1;
        }
        ...
        scale.x = x;
        scale.y = y;
        card_gfx_set_scaled_poly_ft4_geometry_and_uv(poly, &g_card_gfx_scaled_sprite_texture_page,
            &g_card_gfx_draw_area_template, &g_card_gfx_scaled_sprite_uv_rects[i], &scale, sprite->offset);
