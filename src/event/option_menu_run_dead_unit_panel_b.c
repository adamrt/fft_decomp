/*
 * OPTION dead-unit panel, variant B.
 *
 * Twin of option_menu_run_dead_unit_panel_a at 0x801c0304; this one allocates and
 * frees its own 4bpp scratch image, uses the second texture-window table, and lays the
 * border out with 8-pixel instead of 16-pixel corners.
 */
...
void option_menu_run_dead_unit_panel_b(void) {
    RECT texture;
    ...
    pixels = battle_menu_alloc_memory(0x400);

    battle_clear_menu_render_buffer(pixels, 0x400);
    texture.w = 0x80;
    texture.h = 0x10;
    ...
    *dest_stride = 0x80;
    ...
    blit_text_glyph(g_battle_menu_glyph_image, pixels, &g_option_text_glyph_source, &g_option_text_glyph_dest);
    option_text_set_blit_source_rect(0, 8, 8, 0x10);
    option_text_set_blit_destination(0x20, 0);
    blit_text_glyph(g_battle_menu_glyph_image, pixels, &g_option_text_glyph_source, &g_option_text_glyph_dest);
    option_text_set_blit_source_rect(0, 0, 0x28, 8);
    option_text_set_blit_destination(0x28, 0);
    blit_text_glyph(g_battle_menu_glyph_image, pixels, &g_option_text_glyph_source, &g_option_text_glyph_dest);
    option_text_set_blit_source_rect(0, 0x18, 0x28, 8);
    option_text_set_blit_destination(0x28, 8);
    blit_text_glyph(g_battle_menu_glyph_image, pixels, &g_option_text_glyph_source, &g_option_text_glyph_dest);
    ...
    for (index = 0; index < 4; index++) {
        g_option_menu_panel_texture_windows_b[index].x = (texture_u + index) * 0x10;
        g_option_menu_panel_texture_windows_b[index].y = texture_v;
    }
    g_option_menu_panel_texture_windows_b[4].x = texture_u + 0x30;
    g_option_menu_panel_texture_windows_b[4].y = texture_v + 8;

    for (index = 0; index < 7; index++) {
        SetDrawMode(&prim->modes[index], 1, 0, GetTPage(0, 0, 0x1c0, 0), &g_option_menu_panel_texture_windows_b[index]);
    }
    ...
    battle_thread_yield();
    battle_menu_free_memory(pixels);
