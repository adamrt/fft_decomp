...
void attack_panel_run_character_status_thread(void) {
    ...
    screen = (status_panel_buffer_t*)g_attack_panel_frames_a;
    base = (status_panel_buffer_t*)g_attack_panel_frames_a;
    state = (status_panel_slot_state_t*)g_attack_unit_editor_state;
    rect = FRAME_RECT;
    battle_menu_init_numeric_display_frame_primitives((RECT*)rect, &screen->numeric_frame);
    attack_gfx_init_menu_tile_and_line_primitives((status_panel_menu_primitives_t*)screen);
    ...
    for (;;) {
        ...
        for (i = 0; i < 19; i++) {
            if (mode == 0) {
                battle_gfx_init_image_loading((POLY_FT4*)&screen->sprites[i],
                    ...
                    &((world_gfx_image_load_parameters_t*)g_attack_panel_label_layouts_mode0)[i]);
            }
            if (mode == 1) {
                battle_gfx_init_image_loading((POLY_FT4*)&screen->sprites[i],
                    (const battle_image_location_t*)g_attack_editor_numeric_geometry,
                    (const battle_image_location_t*)g_attack_panel_origin_offsets,
                    &((world_gfx_image_load_parameters_t*)g_attack_panel_label_layouts_mode1)[i]);
            }
            if (mode == 2) {
                battle_gfx_init_image_loading((POLY_FT4*)&screen->sprites[i],
                    (const battle_image_location_t*)g_attack_editor_numeric_geometry,
                    (const battle_image_location_t*)g_attack_panel_origin_offsets,
                    &((world_gfx_image_load_parameters_t*)g_attack_panel_label_layouts_mode2)[i]);
            }
            if (mode == 3) {
                battle_gfx_init_image_loading((POLY_FT4*)&screen->sprites[i],
                    (const battle_image_location_t*)g_attack_editor_numeric_geometry,
                    (const battle_image_location_t*)g_attack_panel_origin_offsets,
                    &((world_gfx_image_load_parameters_t*)g_attack_panel_label_layouts_mode3)[i]);
            }
        }
        if (state->generic_monster != 0) {
            for (i = 0; i < 5; i++) {
        ...
        if (frame == 0 || thread->redraw_request == 1) {
            ...
            LoadImage((RECT*)g_attack_panel_text_upload_rect_a, (u32*)g_attack_panel_text_image_a);
            ...
            LoadImage((RECT*)g_attack_panel_text_upload_rect_b, (u32*)g_attack_panel_text_image_b);
        ...
        for (; i < 5; i++) {
            battle_gfx_init_image_loading((POLY_FT4*)&screen->sprites[19 + i],
                ...
                &((world_gfx_image_load_parameters_t*)g_attack_panel_item_icon_layouts)[i]);
            if ((state->equipment[i] & 0xff) != 0xff) {
        ...
        attack_panel_set_primitive_colors((status_panel_primitives_t*)screen, thread);
