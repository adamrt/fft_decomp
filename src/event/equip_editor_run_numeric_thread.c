...
void equip_editor_run_numeric_thread(void) {
    ...
    i = 0;
    descriptor = (const world_gfx_image_load_parameters_t*)g_equip_editor_numeric_texture;
    for (; i < 18; i++) {
    ...
    for (frame = 0;; frame++) {
        ...
        equip_gfx_apply_menu_palette_for_mode(&buffer->numeric_frame, (s32*)thread);
        if (frame < 13) {
