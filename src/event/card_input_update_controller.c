...
void card_input_update_controller(void) {
    ...
    speed = card_menu_get_event_speed();
                ...
                index = 0;
                {
                    ...
                    do {
                        ...
                        if (matched_input != 0) {
                            ...
                            g_card_input_latched_button = bit;
                        }
                    } while (index < 16);
                }
            } else {
    ...
    if (g_card_input_activation_timer >= 10) {
        g_card_input_activation_timer = 10;
    }
    g_card_input_newly_pressed = filtered_input;
    for (index = 0; index < 16; index++) {
        s32 mask = 1 << index;

        if (mask & g_card_input_newly_pressed) {
            if (mask & g_card_input_previous) {
                g_card_input_newly_pressed &= ~mask;
            } else {
                g_card_input_previous |= mask;
            }
        } else {
            g_card_input_previous &= ~mask;
        }
    }
    counter_base = (u32*)g_card_input_repeat_counters;
    index = 0;
    ...
    {
        u32 newly_pressed = g_card_input_newly_pressed & 0x0ff0;
        g_card_input_primary_repeat = (current_input & 0xf00ff00f) | newly_pressed;
        g_card_input_secondary_repeat = (filtered_input & 0xf00ff00f) | newly_pressed;
    }
}
