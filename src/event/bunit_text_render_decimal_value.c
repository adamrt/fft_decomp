...
void bunit_text_render_decimal_value(s32 value, s32 flags, void* pixels, status_panel_text_position_t* position) {
    ...
    if ((render_flags & 0x8000) != 0) {
        render_position->x -= 2;
        g_bunit_text_decimal_glyph.source_x = 0xb4;
        blit_text_glyph(g_battle_menu_glyph_image, render_pixels, &g_bunit_text_decimal_glyph, render_position);
        render_position->x += 7;
    }

    if ((render_flags & 0x0100) != 0) {
        g_bunit_text_decimal_glyph.source_x = 0x78;
        g_bunit_text_decimal_glyph.source_y = 0x1a;
        g_bunit_text_decimal_glyph.height = 4;
        render_position->y += 3;
        blit_text_glyph(g_battle_menu_glyph_image, render_pixels, &g_bunit_text_decimal_glyph, render_position);
        render_position->x += 7;
        render_position->y -= 3;
        g_bunit_text_decimal_glyph.source_y = 0x10;
        g_bunit_text_decimal_glyph.height = 0x0a;
    }

    render_position->color = saved_color;

    if ((render_flags & MENU_DECIMAL_SIGN_MINUS) != 0) {
        g_bunit_text_decimal_glyph.source_x = 0xba;
        if (current_value == 0) {
    ...
    if (saved_color != 0) {
        g_bunit_text_decimal_glyph.source_x = 0xc8;
        blit_text_glyph(g_battle_menu_glyph_image, render_pixels, &g_bunit_text_decimal_glyph, render_position);
        render_position->x += 5;
    }
    ...
    if ((render_flags & 0xff) > 0) {
        glyph = &g_bunit_text_decimal_glyph;
        do {
