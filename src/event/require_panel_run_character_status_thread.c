...
void require_panel_run_character_status_thread(void) {
    ...
    screen = (status_panel_buffer_t*)g_require_panel_frames_a;
    base = (status_panel_buffer_t*)g_require_panel_frames_a;
    state = (status_panel_slot_state_t*)g_require_editor_unit_fields;
    rect = FRAME_RECT;
    battle_menu_init_numeric_display_frame_primitives((RECT*)rect, &screen->numeric_frame);
    require_gfx_init_menu_tile_and_line_primitives((status_panel_menu_primitives_t*)screen);
    ...
    for (;;) {
        ...
        for (i = 0; i < 19; i++) {
            if (mode == 0) {
                battle_gfx_init_image_loading((POLY_FT4*)&screen->sprites[i],
                    ...
                    (const battle_image_location_t*)g_require_panel_origin_offsets,
                    &((world_gfx_image_load_parameters_t*)g_require_panel_label_layouts_mode0)[i]);
            }
            if (mode == 1) {
                battle_gfx_init_image_loading((POLY_FT4*)&screen->sprites[i],
                    (const battle_image_location_t*)g_require_editor_numeric_geometry,
                    (const battle_image_location_t*)g_require_panel_origin_offsets,
                    &((world_gfx_image_load_parameters_t*)g_require_panel_label_layouts_mode1)[i]);
            }
            if (mode == 2) {
                battle_gfx_init_image_loading((POLY_FT4*)&screen->sprites[i],
                    (const battle_image_location_t*)g_require_editor_numeric_geometry,
                    (const battle_image_location_t*)g_require_panel_origin_offsets,
                    &((world_gfx_image_load_parameters_t*)g_require_panel_label_layouts_mode2)[i]);
            }
            if (mode == 3) {
                battle_gfx_init_image_loading((POLY_FT4*)&screen->sprites[i],
                    (const battle_image_location_t*)g_require_editor_numeric_geometry,
                    (const battle_image_location_t*)g_require_panel_origin_offsets,
                    &((world_gfx_image_load_parameters_t*)g_require_panel_label_layouts_mode3)[i]);
            }
        }
        if (state->generic_monster != 0) {
            for (i = 0; i < 5; i++) {
        ...
        if (frame == 0 || thread->redraw_request == 1) {
            battle_clear_menu_render_buffer(g_require_panel_text_image_a, 0xc80);
            ...
            for (i = 0; i < 5; i++) {
                if ((state->equipment[i] & 0xff) != 0xff) {
                    battle_menu_display_text_entry((s16)state->equipment[i] + TEXT_ID_ITEM_NAME_BASE,
                        g_require_panel_text_image_a, &g_menu_text_state.origin_x);
                }
                g_menu_text_state.origin_y += 0x10;
            }
            LoadImage((RECT*)g_require_panel_text_upload_rect_a, (u32*)g_require_panel_text_image_a);
            battle_clear_menu_render_buffer(g_require_panel_text_image_b, 0xc80);
            g_menu_inner_window_width = 0x50;
            battle_menu_set_text_origin(0, 0);
            for (i = 0; i < 2; i++) {
                ...
                if ((u16)state->ability_ids[i] == STATUS_PANEL_LABEL_NONE) {
                    ...
                    battle_menu_display_text_entry(mode, g_require_panel_text_image_b, &g_menu_text_state.origin_x);
                    g_menu_text_state.origin_y += 0x10;
                }
            }
            for (; i < 5; i++) {
                if (state->ability_ids[i] != -1) {
                    battle_menu_display_text_entry(state->ability_ids[i] + TEXT_ID_ABILITY_NAME_BASE,
                        g_require_panel_text_image_b, &g_menu_text_state.origin_x);
                }
                g_menu_text_state.origin_y += 0x10;
            }
            LoadImage((RECT*)g_require_panel_text_upload_rect_b, (u32*)g_require_panel_text_image_b);
        ...
        for (; i < 5; i++) {
            battle_gfx_init_image_loading((POLY_FT4*)&screen->sprites[19 + i],
                (const battle_image_location_t*)g_require_panel_item_icon_texture,
                (const battle_image_location_t*)FRAME_RECT,
                &((world_gfx_image_load_parameters_t*)g_require_panel_item_icon_layouts)[i]);
            if ((state->equipment[i] & 0xff) != 0xff) {
        ...
        mask = scale ^ 0xf0;
        mask = -(mask != 0);
        g_require_gfx_draw_offset_y = mask & 0xf0;
        require_gfx_build_scaled_draw_area_packets(
            &screen->portrait, g_require_character_status_draw_area_rect, frame, scale, (const s16*)thread);
        screen->draw_offsets[0].x = thread->origin_x - 0x80;
        screen->draw_offsets[0].y = thread->origin_y + (u16)g_require_gfx_draw_offset_y;
        SetDrawOffset(&screen->draw_offsets[0], &screen->draw_offsets[0].x);
        screen->draw_offsets[1].x = -0x80;
        screen->draw_offsets[1].y = g_require_gfx_draw_offset_y;
        SetDrawOffset(&screen->draw_offsets[1], &screen->draw_offsets[1].x);
        require_panel_set_primitive_colors((status_panel_primitives_t*)screen, thread);
        require_gfx_apply_menu_palette_for_mode(&screen->numeric_frame, (u8*)thread);
        if (frame < 13) {
