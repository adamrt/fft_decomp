...
/* ATTACK twin of require_load_party_unit_into_editor. Load a party roster unit into the REQUIRE status/editor display.
 *
...
void attack_load_party_unit_into_editor(s32 unused, s32 party_index) {
    battle_stats_t* unit;
    s16* editor;
    u16* innate_items;
    s16* ability_slot;
    ...
    g_attack_editor_selected_unit_id = party_index;
    g_attack_editor_team_state = 0;
    g_attack_editor_job_id = unit->job_id;
    g_attack_editor_brave = unit->brave;
    g_attack_editor_faith = unit->faith;
    g_attack_editor_zodiac = *(u16*)&unit->birthday >> 12;
    if (unit->initial_team_flags & BATTLE_TEAM_MASK) {
        g_attack_editor_team_state = 1;
    }
    ...
    g_attack_editor_status_gauges.max_hp = maximum_hp;
    if (maximum_hp == 0) {
        g_attack_editor_status_gauges.max_hp = maximum_hp + 1;
    }
    g_attack_editor_status_gauges.hp = g_attack_editor_status_gauges.max_hp;
    maximum_mp = unit->max_mp;
    g_attack_editor_status_gauges.max_mp = maximum_mp;
    if (maximum_mp == 0) {
        g_attack_editor_status_gauges.max_mp = maximum_mp + 1;
    }
    g_attack_editor_status_gauges.mp = g_attack_editor_status_gauges.max_mp;
    g_attack_editor_status_gauges.max_ct = 100;
    g_attack_editor_status_gauges.ct = 100;
    ...
    if (editor[7] != 0) {
        ...
        if (field_value == 0)
            g_attack_editor_third_ability_bytes[0] = 0xfe;
        field_value = unit->secondary_skillset;
        editor[24] = field_value;
        if (field_value == 0)
            g_attack_editor_fourth_ability_bytes[1] = 0xfe;
