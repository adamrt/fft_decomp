#include "fft/event_equip.h"
#include "psx/types.h"

/* EQUIP.OUT 001caf14 - Stream command: draw a LINE_F2 from (x0, y0) to
   (x1, y1) with the RGB that follows the endpoints in the command. */
u8* equip_cmd_draw_line_f2_handler(u8* stream) {
    RECT endpoints;

    endpoints.x = stream[2];
    endpoints.y = stream[3];
    endpoints.w = stream[4];
    endpoints.h = stream[5];
    equip_gfx_enqueue_line_f2(&endpoints, stream + 6, (u8)g_equip_gfx_semitransparency, g_equip_gfx_sprite_ot_index);
    return stream + stream[1];
}
