...
u8* card_cmd_draw_tiled_rectangle_handler(u8* command) {
    ...
    otag_index = g_card_text_otag_index;
    ...
    if (kind == 2) {
        ...
        base_x = g_card_gfx_tiled_rect.x;
        width = g_card_gfx_tiled_rect.w;
        base_y = g_card_gfx_tiled_rect.y;
        {
        ...
        height = g_card_gfx_tiled_rect.h;
        full_rows = height / 16;
    ...
    if (full_rows > 0) {
        ...
        y = base_y;
        do {
            semitrans = g_card_gfx_draw_semitrans;
            tpage = g_card_text_digit_texture_page;
            otag = otag_index;
            rect.x = base_x;
            rect.y = y;
            rect.w = partial_width;
            rect.h = 16;
            /* The target passes the clut word without the definition's u16 narrowing. */
            ((void (*)(const RECT*, s32, s32, const u8*, s32, u16, s32, s32))card_gfx_enqueue_textured_quad)(
            ...
            if (full_columns > 0) {
                ...
                x = source_u;
                do {
                    semitrans2 = g_card_gfx_draw_semitrans;
                    tpage2 = g_card_text_digit_texture_page;
                    otag2 = otag_index;
                    rect.x = x;
                    rect.y = tile_y;
                    rect.w = 16;
    ...
    __asm__ __volatile__("" : : "r"(row));
    if (partial_height != 0) {
        semitrans3 = g_card_gfx_draw_semitrans;
        tpage3 = g_card_text_digit_texture_page;
        otag3 = otag_index;
        ...
        card_gfx_enqueue_textured_quad(&rect, 8, 8, 0, semitrans3, tpage3, clut, otag3);
        column = 0;
        if (full_columns > 0) {
            last_y = bottom_y;
            x = base_x + partial_width;
            do {
                semitrans4 = g_card_gfx_draw_semitrans;
                tpage4 = g_card_text_digit_texture_page;
                otag4 = otag_index;
                rect.x = x;
                rect.y = last_y;
                tile_w = 16;
