...
typedef struct effect_timeline_state {
...
} effect_timeline_state_t;

/* The target passes a fourth argument that the sound-timer definition does not declare. */
typedef void (*effect_sound_timer_fn)(u8* schedule, s16* entry_index, s16* countdown, u8* state);
...
s32 battle_effect_code_script_29_step_parent_timeline(effect_record_t* record) {
    ...
    if (state->frame >= g_battle_effect_misc_data->phase1_duration) {
        ...
        ((effect_sound_timer_fn)battle_effect_update_on_hit_sound_timer)(g_battle_effect_misc_data->sound_tracks[0][0],
            &state->track_keyframe[4][0], &state->track_remaining[4][0], &state->sound_state[0][0][1]);
        ((effect_sound_timer_fn)battle_effect_update_on_hit_sound_timer)(g_battle_effect_misc_data->sound_tracks[0][1],
            &state->track_keyframe[5][0], &state->track_remaining[5][0], &state->sound_state[1][0][1]);
        ((effect_sound_timer_fn)battle_effect_update_on_hit_sound_timer)(g_battle_effect_misc_data->sound_tracks[0][2],
            &state->track_keyframe[6][0], &state->track_remaining[6][0], &state->sound_state[2][0][1]);
    ...
    if (state->frame >= i) {
        ...
        ((effect_sound_timer_fn)battle_effect_update_on_hit_sound_timer)(g_battle_effect_misc_data->sound_tracks[1][0],
            &state->track_keyframe[4][1], &state->track_remaining[4][1], &state->sound_state[0][1][1]);
        ((effect_sound_timer_fn)battle_effect_update_on_hit_sound_timer)(g_battle_effect_misc_data->sound_tracks[1][1],
            &state->track_keyframe[5][1], &state->track_remaining[5][1], &state->sound_state[1][1][1]);
        ((effect_sound_timer_fn)battle_effect_update_on_hit_sound_timer)(g_battle_effect_misc_data->sound_tracks[1][2],
            &state->track_keyframe[6][1], &state->track_remaining[6][1], &state->sound_state[2][1][1]);
    }
