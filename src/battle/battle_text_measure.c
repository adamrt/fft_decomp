...
/* BATTLE twin of world_text_measure. Measure a text entry in characters: the widest line and the line count.
 *
...
void battle_text_measure(s16* columns, s16* rows, const u8* text) {
    party_data_t* party;
    u8* cursor;
    s32 i;
    ...
    while (1) {
        ...
        if (state->command == TEXT_NEWLINE || state->command == TEXT_END_WAIT_FOR_CONFIRM
            ...
            if (state->pending_width == 0xFFFF) {
                state->column += battle_text_count_decimal_digits(state->value);
            } else {
