 ...
 *
 * rotation_sum is genuinely uninitialised in the target (s6 is never set). */
void world_script_focus_speed(u8* parameters, s32* position, s32* rotation) {
    s32 position_delta[3];
    s32 rotation_delta[3];
    s32 speed;
    s32 i;
    s32 value;
    s32 position_sum;
    ...
    speed = world_script_load_halfword(parameters);
    /* Skip the speed halfword and the byte after it. */
    parameters += 2;
    parameters++;
    position_sum = 0;
    for (i = 0; i < 3; i++) {
        value = world_script_load_halfword(parameters + i * 2);
        if (value == 10000) {
            position_delta[i] = 0;
        } else {
            position_delta[i] = value - position[i] / 1024;
        }
        value = world_script_load_halfword(parameters + i * 2 + 6);
        if (value == 10000) {
            rotation_delta[i] = 0;
        } else {
    ...
    ((void (*)(u8*, s16))world_script_store_halfword)(parameters + 14, position_sum);
}
