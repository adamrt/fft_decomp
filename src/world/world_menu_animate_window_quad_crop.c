...
void world_menu_animate_window_quad_crop(menu_window_buffer_t* buffers, RECT* rect) {
    ...
    for (level = 0; level < 12; level += g_world_event_speed) {
        ...
        buffers[g_world_menu_packet_buffer_index].quads[0].x1 = sprite.x0 + (sprite.w - 2);
        ...
        buffers[g_world_menu_packet_buffer_index].quads[0].x3 = sprite.x0 + (sprite.w - 2);
        ...
        buffers[g_world_menu_packet_buffer_index].quads[0].u1 = sprite.u0 + (sprite.w - 2);
        ...
        buffers[g_world_menu_packet_buffer_index].quads[0].u3 = sprite.u0 + (sprite.w - 2);
