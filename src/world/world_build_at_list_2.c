 ...
 * 15 running world_menu_run_dead_unit_panel_a_2).
 *
 ...
 * scroll whose new row is drawn into the image ring; page scrolling is left
 * to world_menu_redraw_text_page_on_scroll_2, and the scroll-bar thumb tracks
 * entry->selected_index. The list layout is read into `binding` for sizing and
 * copied to `layout` before the loop: with one variable the allocator gives
 * the image buffer the last callee-saved register instead of the layout.
 */
void world_build_at_list_2(void) {
    ...
    world_menu_text_layout_t* binding;
    s32 delta;
    s32 i;
    s32 column;
    s32 index;
    ...
    binding = (world_menu_text_layout_t*)entry->text_binding;
    entry->window_height = binding->row_count * 16 + 0x1C;
    entry->inner_height = binding->row_count * 16 + 0x1C;
    entry->overall_height = binding->row_count * 16 + 0x1C;
    window.rect.x = 0;
    window.rect.y = 0x7E;
    ...
    for (i = 0; i < 2; i++) {
        g_world_text_list_pages[i].icons.icons[0] = &icons0[i];
        g_world_text_list_pages[i].icons.icons[1] = &icons1[i];
        g_world_text_list_pages[i].icons.icons[2] = &icons2[i];
        world_menu_build_icon_record(
            &image_rect, (world_menu_icon_thread_param_t*)entry, &g_world_text_list_pages[i].icons);
    }
    ...
    redraw_pending = 1;
    world_thread_start(0xF, world_menu_run_dead_unit_panel_a_2);
    ...
    world_script_copy_bytes(&g_world_text_list_pages[1], g_world_text_list_pages, sizeof(world_menu_list_page_t));
    for (i = 0; i < 2; i++) {
        g_world_text_list_pages[i].icons.icons[0] = &icons0[i];
        g_world_text_list_pages[i].icons.icons[1] = &icons1[i];
        g_world_text_list_pages[i].icons.icons[2] = &icons2[i];
    ...
    *input1 = 0;
    for (;;) {
        page = &g_world_text_list_pages[i & 1];
        frame = &page->frame;
        ...
        if (delta == 0 && world_menu_is_input_allowed() != 0 && i != 0) {
            ...
            if (delta != 0) {
                ...
                image_rect.h = 0x10;
                buffer = world_menu_alloc_ui_buffer(0x800);
        ...
        if (delta == 0 && world_menu_is_input_allowed() != 0) {
            buffer = ((u8 * (*)(world_menu_entry_t*, s32*, s32*, s32*)) world_menu_redraw_text_page_on_scroll_2)(
