...
/* Handle item selection and purchase-menu transitions.
 */
void world_shop_handle_item_purchase_menu(void) {
    s16 x;
    s16 y;
    s32 input;
    s32 category;
    ...
    if (g_world_shop_purchase_list_initialized == 0) {
        if (world_thread_is_running(2) == 0) {
            world_thread_start(2, world_text_message_box_thread);
        }
        world_thread_set_parameters(2, 0x19, 0xF804, 0);
        g_world_shop_purchase_list_initialized = 1;
        item_list = g_world_shop_id;
        if (g_world_shop_item_category != 4) {
            /* Preserve the second category read; CSE otherwise removes 12 target bytes. */
            __asm__("" : : : "memory");
        ...
        g_world_shop_purchase_equip_check_pending = 0;
    }
    world_script_set_vsync_mode_and_event_speed(2);
    if (g_world_shop_purchase_equip_check_pending != 0) {
        world_thread_set_parameters(2, 0x19, -1, 0);
        result = world_thread_is_running(2);
        g_world_shop_purchase_equip_check_pending = result;
        if (result == 0) {
            g_world_shop_purchase_list_initialized = 0;
        ...
        if (input & PSX_PAD_CROSS) {
            ...
            g_world_shop_purchase_equip_check_pending = 1;
    ...
    world_menu_run_script_with_palette_mode(
        g_world_shop_purchase_menu_script, g_world_input_primary_repeat, g_world_thread_task_active);
