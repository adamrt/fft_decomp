...
void world_menu_init_system_function(s32 command, s32 option, s32 unit_id, s32 parameter, s32 enabled) {
    struct battle_ai_command_action* action;

    if (command != 8 && command != 6) {
        action = (struct battle_ai_command_action*)battle_unit_get_target_id_ptr_by_battle_id(unit_id);
        if (command == 5) {
            world_text_determine_spell_quote((world_unit_command_action_t*)action, unit_id, enabled);
            return;
        }
        if (command == 7) {
            world_menu_dispatch_system_function(7, 0, unit_id, 0, 0, action);
            return;
        }
    }
    world_menu_dispatch_system_function(command, option, unit_id, parameter, enabled, action);
}
