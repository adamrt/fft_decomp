/*
 * WORLD copy of the EVENT OPTION dead-unit panel (option_menu_run_dead_unit_panel_b; second WORLD copy with its own
 * blit descriptors, blit setters and texture-window table (the frame block at 0x801cc074 is shared with
...
void world_menu_run_dead_unit_panel_b_2(void) {
    RECT texture;
    ...
    *dest_stride = 0x80;
    ...
    world_text_blit_glyph(g_world_menu_glyph_sheet, pixels, &g_world_text_glyph_source, &g_world_text_glyph_dest);
    world_gfx_set_blit_source_rect(0, 0, 0x28, 8);
    world_gfx_set_blit_dest_position(0x28, 0);
    world_text_blit_glyph(g_world_menu_glyph_sheet, pixels, &g_world_text_glyph_source, &g_world_text_glyph_dest);
    world_gfx_set_blit_source_rect(0, 0x18, 0x28, 8);
    world_gfx_set_blit_dest_position(0x28, 8);
    world_text_blit_glyph(g_world_menu_glyph_sheet, pixels, &g_world_text_glyph_source, &g_world_text_glyph_dest);
    ...
    for (index = 0; index < 4; index++) {
        g_world_dead_unit_panel_b_texture_windows[index].x = (texture_u + index) * 0x10;
        g_world_dead_unit_panel_b_texture_windows[index].y = texture_v;
    }
    g_world_dead_unit_panel_b_texture_windows[4].x = texture_u + 0x30;
    g_world_dead_unit_panel_b_texture_windows[4].y = texture_v + 8;

    for (index = 0; index < 7; index++) {
        SetDrawMode(
            &prim->modes[index], 1, 0, GetTPage(0, 0, 0x1c0, 0), &g_world_dead_unit_panel_b_texture_windows[index]);
    }
