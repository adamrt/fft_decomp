...
s32 main_ability_calculate_pointers_and_type(s32 id, u8** out_ability_data, u8** out_secondary_data) {
    ...
    *out_ability_data = (u8*)&g_main_ability_data[id];
    if (id < ABILITY_ID_ITEM_FIRST) {
        *out_secondary_data = (u8*)&g_main_ability_range_data[id];
        return ABILITY_TYPE_DEFAULT;
    }
    if (id < ABILITY_ID_THROW_FIRST) {
        *out_secondary_data = &g_main_item_ability_item_ids_by_ability_id[id];
        return ABILITY_TYPE_ITEM;
    }
    if (id < ABILITY_ID_JUMP_FIRST) {
        *out_secondary_data = &g_main_throw_ability_item_types_by_ability_id[id];
        return ABILITY_TYPE_THROW;
    }
    if (id < ABILITY_ID_CHARGE_FIRST) {
        *out_secondary_data = &g_main_jump_charge_ability_data_by_ability_id[id * 2];
    ...
    if (id < ABILITY_ID_REACTION_FIRST) {
        *out_secondary_data = &g_main_math_rsm_ability_data_by_ability_id[id];
