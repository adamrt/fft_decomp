...
#define RIBBON_SLOTS    EFFECT_PARTICLE_RIBBON_LONG_SLOTS
#define RIBBON_POINTS   EFFECT_PARTICLE_RIBBON_LONG_POINTS
...
s32 effect_e463_update_particle_ribbons_long_state(s16 record_index, s32 byte_offset, s32 geometry_index, s32 frame) {
    SVECTOR target_base;
    ...
    effect_particle_ribbon_long_work_t* work;
    ...
    s16 mid;
    s32 bisector;
    effect_particle_ribbon_long_t* slot;
    POLY_GT4* quad;
    POLY_GT4* prev;
    map_tile_t* tile;
    ...
    case EFFECT_PHASE_INIT:
        ...
        work = battle_heap_alloc_block(sizeof(effect_particle_ribbon_long_work_t), record_index);
    ...
    case EFFECT_PHASE_UPDATE:
        ...
        if (i % battle_effect_lerp_linear(entry->spawn_interval_start, entry->spawn_interval_end, factor) == 0) {
            ...
            for (i = 0; i < count; i++) {
                ...
                if (largest != 0) {
                    ...
                    vector.vz = 0;
                    vector.vy = 0;
                    gte_RotTrans(&vector, &result, &result.pad);
        ...
        for (k = 0; k < RIBBON_SLOTS; k++) {
            ...
            quad->v0 = v;
            quad->v2 = v + height;
            factor = g_effect_e461_particle_ribbon_long_brightness_table[row][point];
