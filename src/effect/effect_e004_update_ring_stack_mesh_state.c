 ...
 * lattice row takes its brightness from g_effect_e004_ring_stack_mesh_brightness_table.
 *
...
s32 effect_e004_update_ring_stack_mesh_state(s32 record_index, s32 byte_offset, s32 geometry_index, s32 frame) {
    SVECTOR vertex;
    ...
    case EFFECT_PHASE_UPDATE:
        ...
        k = g_effect_e004_ring_stack_mesh_brightness_table[n][0];
        for (i = 0; i < 8; i++) {
            quad = &work->quads[buffer][i];
            quad->r3 = (k * red) >> 12;
            quad->r2 = (k * red) >> 12;
        ...
        for (j = 0; j < 7; j++) {
            k = g_effect_e004_ring_stack_mesh_brightness_table[n][j + 1];
            for (i = 0; i < 8; i++) {
        ...
        k = g_effect_e004_ring_stack_mesh_brightness_table[n][8];
        for (i = 0; i < 8; i++) {
