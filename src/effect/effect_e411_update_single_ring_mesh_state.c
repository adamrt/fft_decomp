...
s32 effect_e411_update_single_ring_mesh_state(s32 record_index, s32 byte_offset, s32 geometry_index, s32 frame) {
    SVECTOR vertex;
    ...
    case EFFECT_PHASE_UPDATE:
        ...
        k = g_effect_single_ring_mesh_brightness_table[n][0];
        for (i = 0; i < RING_MESH_SEGMENTS; i++) {
            quad = &work->quads[buffer][i];
            quad->r3 = (k * red) >> 12;
            quad->r2 = (k * red) >> 12;
        ...
        k = g_effect_single_ring_mesh_brightness_table[n][1];
        for (i = 0; i < RING_MESH_SEGMENTS; i++) {
            quad = &work->quads[buffer][i];
            quad->r1 = (k * red) >> 12;
            quad->r0 = (k * red) >> 12;
