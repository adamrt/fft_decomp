 ...
 * and every lattice row takes its brightness from g_effect_e071_summon_mesh_brightness_table. The quads are
...
s32 effect_e071_update_summon_mesh_state(s16 record_index, s32 byte_offset, s32 geometry_index, s32 frame) {
    SVECTOR vertex;
    ...
    case EFFECT_PHASE_UPDATE:
        ...
        k = g_effect_e071_summon_mesh_brightness_table[n][0];
        shade_r = (k * red) >> 12;
        shade_g = (k * green) >> 12;
        shade_b = (k * blue) >> 12;
        for (j = 0; j < 16; j++) {
        ...
        for (i = 0; i < 7; i++) {
            k = g_effect_e071_summon_mesh_brightness_table[n][i + 1];
            shade_r = (k * red) >> 12;
            shade_g = (k * green) >> 12;
            shade_b = (k * blue) >> 12;
            for (j = 0; j < 16; j++) {
        ...
        k = g_effect_e071_summon_mesh_brightness_table[n][8];
        shade_r = (k * red) >> 12;
        shade_g = (k * green) >> 12;
        shade_b = (k * blue) >> 12;
        for (j = 0; j < 16; j++) {
