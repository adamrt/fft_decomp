 ...
 *
 * Nine lattice rows of eight points are mirrored about row 4: g_effect_e383_tube_mesh_ring_scales
 ...
 * and every lattice row takes its brightness from g_effect_e383_tube_mesh_brightness_table.
 *
 ...
 * row and quad depth). */
s32 effect_e383_update_tube_mesh_state(s32 record_index, s32 byte_offset, s32 geometry_index, s32 frame) {
    SVECTOR vertex;
    ...
    case EFFECT_PHASE_UPDATE:
        ...
        for (i = 0; i < 5; i++) {
            extent = (((cos0 * g_effect_e383_tube_mesh_ring_scales[i]) >> 12) * (radius + (work->radius_offset >> 8)))
                >> 12;
            ...
            extent = (((sin0 * g_effect_e383_tube_mesh_ring_scales[i]) >> 12) * (radius + (work->radius_offset >> 8)))
                >> 12;
            ...
            extent = (((cos45 * g_effect_e383_tube_mesh_ring_scales[i]) >> 12) * (radius + (work->radius_offset >> 8)))
                >> 12;
            ...
            extent = (((sin45 * g_effect_e383_tube_mesh_ring_scales[i]) >> 12) * (radius + (work->radius_offset >> 8)))
                >> 12;
        ...
        k = g_effect_e383_tube_mesh_brightness_table[n][0];
        shade_r = (k * red) >> 12;
        shade_g = (k * green) >> 12;
        shade_b = (k * blue) >> 12;
        for (i = 0; i < 8; i++) {
        ...
        for (j = 0; j < 7; j++) {
            k = g_effect_e383_tube_mesh_brightness_table[n][j + 1];
            shade_r = (k * red) >> 12;
            shade_g = (k * green) >> 12;
            shade_b = (k * blue) >> 12;
            for (i = 0; i < 8; i++) {
        ...
        k = g_effect_e383_tube_mesh_brightness_table[n][8];
        shade_r = (k * red) >> 12;
        shade_g = (k * green) >> 12;
        shade_b = (k * blue) >> 12;
        for (i = 0; i < 8; i++) {
